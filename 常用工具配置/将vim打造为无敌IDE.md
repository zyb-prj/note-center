...
当前阶段，ubuntu18 LTS 依旧为主力。使用 vim 需要各种插件，在配置文件中设置的插件更新也会默认获取最新版。官网安装的 ubuntu18 LTS 默认自带 python3.6，同时使用 `sudo apt-get install vim` 安装的 vim 版本是 8.0。在后续安装部分好用的插件时会各种报错。以下为老夫亲自含泪整理的一些踩坑说明：
...
- 安装代码自动补全插件 YouCompleteMe
    ...
    - 编译过程至少需要 python3.8+。
      ...
      - ubuntu18 LTS 自带 python3.6，且其中各种依赖较多，因此千万不要动 python3 这个软连接。
        - 用户安装完高版本 python（假设为 python3.10）之后直接做一个 python3.10 的软连接，然后相对应的在 .vimrc 中修改并编译插件即可。
        - 通过源码安装高版本 python（python3.10）时要求 cmake 安装版本至少是 3.15。通过源码安装高版本 cmake 又需要支持 c++17 的 gcc 以及 g++。
...
我们这么折腾的目的是为了安装一些好用的插件，其中最神奇的为安装代码自动补全插件 YouCompleteMe，恰好这个安装最折腾人，几乎所有的坑都是在这个插件的安装上面。首先 vim 本身的版本必须要 8.0+ 且 8.0 不行。不管是哪个版本的 ubuntu 发行版，均可以使用以下方法下载最新版本的 vim。步骤如下：
...
- 如果您想详细了解每一个插件的作用，请参考附录中的[插件安装](#5-3_插件安装)章节。

### 3-3-1_安装步骤

- 1st：启动 vim
- 2nd：vim 进入命令行模式，输入指令`PluginInstall`即可自动安装所有插件。

### 3-3-2_YCM的安装

- YouCompleteMe 插件用于自动补全代码。此插件通过在 .vimrc 中的配置，然后使用指令`PluginInstall`完成安装后并不能直接使用，没有真正的安装好，需要下载最新版本的源码编译再安装。在编译过程中，需要 python3.10 的环境，需要支持 c++17 的 gcc 以及 g++，需要高于 3.15 版本的 cmake。
- 如果您使用的是 Ubuntu18 LTS，编译 YouCompleteMe 插件源码需要 python3.10 ，同时后续使用过程也需要 python3.10 来催动插件。
  - 通过源码编译并安装 python3.10 需要安装高于 3.15 版本的 cmake，Ubuntu18 只能通过源码编译并安装高版本 cmake。
  - 编译 YouCompleteMe 插件源码需要支持 c++17 的 gcc 以及 g++，Ubuntu18 需要安装 gcc8 以及 g++8。

**注意**：Ubuntu18下安指定版本的python，cmake，gcc，g++，请查阅此模块，全部完成后再做下面的操作。此步骤专门用于ubuntu18，更高版本的 ubuntu 发行版不需要此步骤。请按照[安装指定版本的软件](#5-1_安装指定版本的软件)所示的步骤安装完成。
...
- 3rd 在 YCM 源码根目录编译源码，一定要用做好软连接的 python3.10 编译。
...
- 再次打开 vim 使用`:PluginInstall`指令重新安装插件。
...
- 1st 使用命令行安装，此处你可以使用 Tab 键来补充指定的 gcc 以及 g++ 版本，亲测最新是 8.4 版本，满足要求。
...
- 3rd 操作过安装指令后，可执行文件位于 /usr/local/bin 中，我们需要做个 python3.10 的软连接方便后续操作。千万不要直接链接到 python3，否则后面很痛苦。

## 5-2_配置vim详解

### 5-2-1_自动安装插件并对插件做使用快捷键配置
